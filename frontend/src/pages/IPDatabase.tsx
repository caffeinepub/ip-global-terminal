...
import {
  Search, Loader2, Database, Plus, Pencil, Trash2,
  ChevronUp, ChevronDown, Globe, Building2, MapPin, Calendar, Shield,
} from 'lucide-react';
...
import EditIPRecordModal, { type EditableRecord } from '../components/EditIPRecordModal';
import DeleteIPRecordConfirmation from '../components/DeleteIPRecordConfirmation';
import { mockIPDatabase, type MockIPRecord } from '../data/mockIPDatabase';
...
function toDisplayRecord(r: IPDatabaseRecord, isBackend: boolean): DisplayRecord {
  return {
...
export default function IPDatabase() {
  ...
  // Backend records
  const { data: backendRecords, isLoading: backendLoading } = useGetAllIPRecords();
  ...
  const allDisplayRecords = useMemo(() => {
    const backendDisplayed = (backendRecords ?? []).map((r) => toDisplayRecord(r, true));
    const backendIPs = new Set(backendDisplayed.map((r) => r.ipAddress));
    const filteredMock = localMockRecords.filter((r) => !backendIPs.has(r.ipAddress));
    return [...backendDisplayed, ...filteredMock];
  }, [backendRecords, localMockRecords]);

  // Unique countries for filter dropdown
  const uniqueCountries = useMemo(() => {
    const countries = Array.from(new Set(allDisplayRecords.map((r) => r.country))).sort();
    return countries;
  }, [allDisplayRecords]);

  // Unique orgs for filter dropdown
  const uniqueOrgs = useMemo(() => {
    const orgs = Array.from(new Set(allDisplayRecords.map((r) => r.owner))).sort();
    return orgs;
  }, [allDisplayRecords]);
      ...
      setLocalMockRecords((prev) =>
        prev.map((r) =>
          r.ipAddress === updated.ipAddress
            ? { ...r, owner: updated.owner, country: updated.country, city: updated.city, registrationDate: updated.registrationDate, status: updated.status }
            : r
        )
      );
    }
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    if (deleteTarget.isBackend) {
      ...
      setLocalMockRecords((prev) => prev.filter((r) => r.ipAddress !== deleteTarget.ipAddress));
      setDeleteTarget(null);
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm" style={{ color: 'oklch(0.55 0 0)' }}>
            Browse IP address records and blockchain-registered intellectual property.
          </p>
        </div>
            ...
            {backendLoading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-lg font-medium" style={{ color: 'oklch(0.55 0 0)' }}>
                  {hasDbFilters ? 'No records match your filters.' : 'No IP records found.'}
                </p>
              </div>
            ) : (
              <div
              ...
              >
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5"><Building2 className="w-3.5 h-3.5" />Owner / Org</span>
                  <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" />Location</span>
                  <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" />Registered</span>
                  <span className="flex items-center gap-1.5"><Shield className="w-3.5 h-3.5" />Status</span>
                  <span>Actions</span>
                </div>
        ...
        {activeTab === 'ip-registry' && (
          <>
            ...
            {isRegistryLoading ? (
              ...
              <>
                <p className="text-xs mb-4" style={{ color: 'oklch(0.45 0 0)' }}>
                  {ipRegistryRecords.length} record{ipRegistryRecords.length !== 1 ? 's' : ''} found
                </p>
...
function IPDatabaseRow({ record, isAuthenticated, onEdit, onDelete }: IPDatabaseRowProps) {
  const dateStr = new Date(record.registrationDate).toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'short',
    year: 'numeric',
  });
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:grid grid-cols-[2fr_2fr_1.5fr_1fr_1.2fr_auto] gap-4 items-center">
        <span className="font-mono text-sm" style={{ color: 'oklch(0.78 0.15 85)' }}>
          {record.ipAddress}
          {record.isBackend && (
            <span
              className="ml-2 text-xs px-1.5 py-0.5 rounded"
              style={{ backgroundColor: 'oklch(0.78 0.15 85 / 0.12)', color: 'oklch(0.78 0.15 85 / 0.70)', fontSize: '0.65rem' }}
            >
              on-chain
            </span>
          )}
        </span>
        <span className="text-sm truncate" style={{ color: 'oklch(0.80 0 0)' }}>{record.owner}</span>
        <span className="text-sm" style={{ color: 'oklch(0.65 0 0)' }}>
          {record.city}, {record.country}
        </span>
        <span className="text-xs" style={{ color: 'oklch(0.50 0 0)' }}>{dateStr}</span>
        ...
        {isAuthenticated ? (
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className="p-1.5 rounded-lg transition-colors"
              style={{ color: 'oklch(0.55 0 0)' }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = 'oklch(0.70 0.20 25)'; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = 'oklch(0.55 0 0)'; }}
              title="Delete record"
            >
      ...
      <div className="md:hidden space-y-2">
        <div className="flex items-start justify-between gap-2">
          <span className="font-mono text-sm font-semibold" style={{ color: 'oklch(0.78 0.15 85)' }}>
            {record.ipAddress}
          </span>
          <StatusBadge status={record.status} />
        </div>
        <div className="text-sm" style={{ color: 'oklch(0.75 0 0)' }}>{record.owner}</div>
