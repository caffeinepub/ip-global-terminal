...
export default function EditIPRecordModal({ open, record, onClose, onSuccess }: EditIPRecordModalProps) {
  ...
  useEffect(() => {
    if (record) {
      ...
      setCity(record.city);
      setRegistrationDate(new Date(record.registrationDate).toISOString().split('T')[0]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (record.isBackend) {
      ...
      try {
        await updateMutation.mutateAsync({ ipAddress: record.ipAddress, updatedRecord: backendRecord });
        onSuccess(updatedRecord);
        onClose();
      } catch (err: any) {
        setErrors({ submit: err?.message ?? 'Failed to update record. Please try again.' });
      }
    } else {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-7">
          ...
          >
            <Pencil className="w-5 h-5" style={{ color: 'oklch(0.78 0.15 85)' }} />
          </div>
          <div>
            <h2
            ...
            <p className="text-xs font-mono" style={{ color: 'oklch(0.50 0 0)' }}>
              {record.ipAddress}
            </p>
          </div>
        </div>
