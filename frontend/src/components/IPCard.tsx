...
export default function IPCard({ record, onClick }: IPCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-black border border-gold-900/30 hover:border-gold-600/50 rounded-sm p-5 cursor-pointer transition-all duration-200 hover:shadow-gold group"
    >
      {/* Header row */}
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <span className={`text-xs font-medium px-2 py-0.5 rounded-sm flex-shrink-0 ${categoryColor(record.category)}`}>
      ...
      <div className="border-t border-gold-900/20 pt-3">
        <div className="flex items-center gap-2">
          <span className="text-xs text-white/30 uppercase tracking-wider">SHA-256</span>
          <span className="font-mono text-xs text-gold-600/80 truncate">{shortHash}</span>
        </div>
      </div>
    </div>
  );
